...
1. Выполните `npm start` для запуска сервера.
2. По умолчанию сервер слушает <http://localhost:3000>.
...
1. Пользовательский счёт с длинной историей переводов:
...
В дальнейшем токен указывается в заголовке Authorization для методов, которые требуют авторизации: `Authorization: Basic TOKEN`ю.
